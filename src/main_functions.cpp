...
// main function for loss errors 
// with peeling decoder
int main_with_loss_error()
...
// main function for loss errors
// with peeling decoder
// for multiple times for plotting

// // run for multiple error rate for plotting data
int main_with_loss_error_for_plot(){
    std::random_device rd;
    std::mt19937 engine(rd());
    std::uniform_real_distribution<double> dist(0,1);
...
// main function for the combined error model (random Z error -> erasure error -> random Z error)
// with union find decoder
int main_with_combined_error()
{   // lattice size: l1 times l2
    int l1,l2, multiplexing;
    l1 = 5;
    l2 = 5;
    multiplexing = 1; // number of qubits per one photon
    std::cout << "\nlattice size              :" << l1 << "x" << l2;
    std::cout << "\nmultiplexing              :" << multiplexing;
    // random device
    std::random_device rd;
    std::mt19937 engine(rd());
    ...
    photons = assign_with_strategy(l1, l2, multiplexing, num_photons, num_qubits, strategy, engine, dist);

    std::cout << "\nphotons                   :";
    print_vec_of_vec(photons);
    std::size_t size = photons.size();
    std::cout << "\nnumber of photons         :" << size;
    if (size != num_photons){
        std::cout << "\nError! number of photons is not num_photons";
    }
    // check assignment works correctly 
    ...
    std::vector<bool> photon_loss;
    int error_model;
    // error_model = 0: random erasure error
    // error_model = 1: burst erasure error
    error_model = 0;
    if (error_model == 0){
        float prob_e;
        prob_e = 0;
    ...
    float prob_z = 0.05;
    // random Z error before communication
    std::vector<bool> random_z_errors_before;
    std::cout << "\nrandom Z err prob (before):" << prob_z;
    random_z_errors_before = random_z_error(num_qubits, prob_z, engine, dist);
    std::cout << "\nrandom Z err (before)     :";
    print_ind_of_bool_vec(random_z_errors_before);
    // cancel Z errors on lost qubits
    for (int q = 0; q < num_qubits; q++){
        if (qubit_loss[q]){
            random_z_errors_before[q] = false;
        }
    }
    // random Z error after communication
    std::vector<bool> random_z_errors_after;
    std::cout << "\nrandom Z err prob (after) :" << prob_z;
    random_z_errors_after = random_z_error(num_qubits, prob_z, engine, dist);
    std::cout << "\nrandom Z err (after)      :";
    print_ind_of_bool_vec(random_z_errors_after);

    std::vector<bool> zerror_before_communication_plus_loss, combined_errors;
    zerror_before_communication_plus_loss = combine_errors(zerrors, random_z_errors_before);
    ...
    // vector of x stabilizers which returns -1
    std::vector<bool> x_syndromes;
    x_syndromes = x_stab_measurement(l1, l2, x_stabs, combined_errors);
    std::cout << "\nX syndromes               :";
    print_ind_of_bool_vec(x_syndromes);
    ...
    std::vector<bool> z_errors_after_decoding(num_qubits);
    for (int qubitind = 0; qubitind < num_qubits; qubitind++){
        z_errors_after_decoding[qubitind] = combined_errors[qubitind] ^ z_correction[qubitind];
    }
    std::cout << "\nZ errors after correction :";
    print_ind_of_bool_vec(z_errors_after_decoding);
    ...
    x_syndromes_after_dec = x_stab_measurement(l1, l2, x_stabs,z_errors_after_decoding);
    std::string filename = "result.json";
    // std::string filename = ss.str() + std::to_string(l1) + "_" + std::to_string(l2) + "_" + std::to_string(multiplexing) + "_" + std::to_string(prob_e) + "_" + "combined.json";
    save_results(filename, l1, l2, num_qubits, num_photons, multiplexing, strategy, photons, photon_loss, qubit_loss, zerrors, x_syndromes, x_pcm, z_correction, z_errors_after_decoding, x_syndromes_after_dec, x_1, x_2, is_success);
    std::cout << "\n";
    return 0;
}

// multiple shots for plotting with union find decoder
int main_with_combined_error_multiple_shots(){
    std::random_device rd;
    std::mt19937 engine(rd());
    std::uniform_real_distribution<double> dist(0,1);
        ...
        for (int strategy: strategies){
            ...
            for (float prob_e = 0.0000; prob_e < 1.0500; prob_e = prob_e + 0.0500){
                ...
                for (int i = 0; i < num_shots; i++){
                    ...
                    std::vector<bool> random_z_errors;
                    float prob_z = 0.03;
                    random_z_errors = random_z_error(num_qubits, prob_z, engine, dist);
                    std::cout << "\nrandom Z errors           :";
                    print_ind_of_bool_vec(random_z_errors);

                    std::vector<bool> combined_errors;
                    combined_errors = combine_errors(zerrors, random_z_errors);
                    std::cout << "\ncombined Z errors         :";
                    print_ind_of_bool_vec(combined_errors);
