...
// multiple shots for plotting logical error rate for erasure error withpeeling decoder
...
// multiple shots for plotting logical error rate for combined error with union find decoder
int main_with_combined_error_for_plot(float prob_z){
    std::random_device rd;
    std::mt19937 engine(rd());
    std::uniform_real_distribution<double> dist(0,1);
    ...
    for (int lattice_size: lattice_sizes){
        ...
        for (int strategy: strategies){
            ...
            for (float prob_e = 0.0000; prob_e < 0.4000; prob_e = prob_e + 0.0100){
                ...
                success_rate.push_back(num_success_fail);
                std::ofstream writing_file;
                std::string filename = "result_" + std::to_string(l1) + "_" + std::to_string(l2) + "_" + std::to_string(strategy) + "_" + std::to_string(multiplexing) + "_" + std::to_string(num_shots) + "_" + std::to_string(prob_z) + ".json";
                if (strategy == 5 || strategy == 6){
                    filename = "result_" + std::to_string(l1) + "_" + std::to_string(l2) + "_" + std::to_string(strategy) + "_" + std::to_string(multiplexing) + "_" + std::to_string(num_shots) + "_f" +std::to_string(force) + ".json";
                }
                auto now = std::chrono::system_clock::now();
