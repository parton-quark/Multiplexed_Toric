...
int pick_leaf_edge(graph G, int l1, int l2){
    edges = G.edges;
    vertices = G.vertices;

    for (edge: edges){
        std::vector<int> uv;
        uv = edge_to_vertices(int l2, int e);
        u = uv[0];
        v = uv[1];
...
std::vector<bool> peeling_decoder_for_z_errors(int l1, int l2, std::vector<bool> erased_qubits, std::vector<bool> x_syndrome){
    ...
    msf = spanning_forest(epsilon, int l2);

    std::vector<bool> p_z /* Z correction*/
    ...
    for (graph mst: msf){ 
        ...
        while (f_eps.num_edges() != 0){
            ...
            leaf = pick_leaf_edge(f_eps, l1, l2);
            // remove e from f_eps
